...
func registerControllers(r *gin.Engine) {
	v1 := r.Group("/api/github.com/fullstack-lang/gongthree/go")
	{ // insertion point for registrations
		v1.GET("/v1/beziercurves", GetController().GetBezierCurves)
		v1.GET("/v1/beziercurves/:id", GetController().GetBezierCurve)
		v1.POST("/v1/beziercurves", GetController().PostBezierCurve)
		v1.PATCH("/v1/beziercurves/:id", GetController().UpdateBezierCurve)
		v1.PUT("/v1/beziercurves/:id", GetController().UpdateBezierCurve)
		v1.DELETE("/v1/beziercurves/:id", GetController().DeleteBezierCurve)

		v1.GET("/v1/beziersegments", GetController().GetBezierSegments)
		v1.GET("/v1/beziersegments/:id", GetController().GetBezierSegment)
		v1.POST("/v1/beziersegments", GetController().PostBezierSegment)
		v1.PATCH("/v1/beziersegments/:id", GetController().UpdateBezierSegment)
		v1.PUT("/v1/beziersegments/:id", GetController().UpdateBezierSegment)
		v1.DELETE("/v1/beziersegments/:id", GetController().DeleteBezierSegment)
...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	// log.Println("Stack github.com/fullstack-lang/gongthree/go, onWebSocketRequestForBackRepoContent")
	...
	log.Printf("Stack github.com/fullstack-lang/gongthree/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gongthree/go, Unkown stack", stackPath)
	}
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gongthree/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gongthree/go:\n",
			"client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	} else {
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gongthree/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gongthree/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
